...
class MinaQrcodeCommand extends Command {
  ...
  async genQrcode() {
    ...
    const _path = encodeURIComponent(encodeURIComponent(encodeURIComponent(
      appPath
    )))
    const params = {
      path: `/wxopen/wxaqrcode?action=getqrcode&f=json&appid=${appId}&path=${_path}&token=${userInfo.token}&lang=zh_CN`,
